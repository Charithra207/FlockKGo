...
export default function Survey() {
  ...
  const [form, setForm] = useState({ budget_min: 500, budget_max: 1500, vibes: [], climate: 'any', activity_level: 'moderate', available_dates: [null, null], exclusions: [], visited: [] })
  ...
  const handleSubmit = async () => {
    try {
      if (!DEV_MODE) await submitSurvey(token, {
        budget_min: form.budget_min,
        budget_max: form.budget_max,
        vibes: form.vibes,
        climate_pref: form.climate,
        activity_level: form.activity_level,
        available_start: form.available_dates[0] ? new Date(form.available_dates[0]).toISOString().slice(0, 10) : null,
        available_end: form.available_dates[1] ? new Date(form.available_dates[1]).toISOString().slice(0, 10) : null,
  ...
  return (
    <div className="mx-auto max-w-3xl rounded-2xl bg-white p-6 shadow-card">
      ...
      {step === 3 && <div className="space-y-4"><h2 className="text-xl font-bold">Climate & Activity</h2><div className="flex gap-2">{['warm', 'cold', 'any'].map((v) => <button key={v} onClick={() => setForm({ ...form, climate: v })} className={`rounded-xl border px-3 py-2 ${form.climate === v ? 'border-accent bg-red-50' : ''}`}>{v}</button>)}</div><div className="flex gap-2">{['relaxed', 'moderate', 'intense'].map((v) => <button key={v} onClick={() => setForm({ ...form, activity_level: v })} className={`rounded-xl border px-3 py-2 ${form.activity_level === v ? 'border-accent bg-red-50' : ''}`}>{v}</button>)}</div><DateRangePicker startDate={form.available_dates[0]} endDate={form.available_dates[1]} onChange={(dates) => setForm({ ...form, available_dates: dates })} /></div>}
